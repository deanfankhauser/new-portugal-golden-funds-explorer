...
import {
  ...
  generateWeeklyDigestEmailUnverified,
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const body = await req.json() as WeeklyDigestRequest;
    ...
    for (const fund of funds || []) {
      try {
        ...
        console.log(`Fund ${fund.name}: ${impressions} impressions, ${leads} leads`);
        ...
        for (const email of managerEmails) {
          ...
          const { html, text } = emailTemplate({
            fundName: fund.name,
            weeklyImpressions: impressions || 0,
            weeklyLeads: leads || 0,
          ...
          const subject = `Weekly Performance Summary - ${fund.name}`;
          ...
          await supabase.from('fund_manager_email_logs').insert({
            ...
            weekly_impressions: impressions || 0,
            weekly_leads: leads || 0,
    ...
    console.log(`Weekly digest completed: ${successCount} sent, ${errorCount} errors`);
