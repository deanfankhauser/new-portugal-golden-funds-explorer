...
BEGIN
  IF EXISTS (
    ...
    ALTER VIEW public.manager_profiles_public SET (security_invoker = true);
...
COMMENT ON VIEW public.manager_profiles_public IS 'Security hardened: SECURITY INVOKER enabled to ensure caller RLS is applied (fix for linter rule 0010).';