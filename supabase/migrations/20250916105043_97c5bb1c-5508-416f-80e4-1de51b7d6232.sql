-- Complete dev database setup with all missing tables and data
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'manager_status') THEN
...
-- Create policies (idempotent with DO blocks)
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'manager_profiles' AND policyname = 'Managers can view own profile') THEN
    CREATE POLICY "Managers can view own profile" ON public.manager_profiles FOR SELECT USING (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'manager_profiles' AND policyname = 'Managers can create own profile') THEN
    CREATE POLICY "Managers can create own profile" ON public.manager_profiles FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'manager_profiles' AND policyname = 'Public marketing info only - no contact details') THEN
    CREATE POLICY "Public marketing info only - no contact details" ON public.manager_profiles FOR SELECT 
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'investor_profiles' AND policyname = 'Investors can view own profile') THEN
    CREATE POLICY "Investors can view own profile" ON public.investor_profiles FOR SELECT USING (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'investor_profiles' AND policyname = 'Investors can create own profile') THEN
    CREATE POLICY "Investors can create own profile" ON public.investor_profiles FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'investor_profiles' AND policyname = 'Investors can update own profile') THEN
    CREATE POLICY "Investors can update own profile" ON public.investor_profiles FOR UPDATE USING (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'fund_edit_suggestions' AND policyname = 'Users can view own suggestions') THEN
    CREATE POLICY "Users can view own suggestions" ON public.fund_edit_suggestions FOR SELECT USING (auth.uid() = user_id);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'fund_edit_suggestions' AND policyname = 'Authenticated users can create suggestions') THEN
    CREATE POLICY "Authenticated users can create suggestions" ON public.fund_edit_suggestions FOR INSERT 
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'fund_edit_suggestions' AND policyname = 'Admins can view all suggestions') THEN
    CREATE POLICY "Admins can view all suggestions" ON public.fund_edit_suggestions FOR SELECT USING (is_user_admin());
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'fund_edit_suggestions' AND policyname = 'Admins can update suggestions') THEN
    CREATE POLICY "Admins can update suggestions" ON public.fund_edit_suggestions FOR UPDATE USING (is_user_admin());
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'fund_edit_history' AND policyname = 'Anyone can view fund edit history') THEN
    CREATE POLICY "Anyone can view fund edit history" ON public.fund_edit_history FOR SELECT USING (true);
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'fund_edit_history' AND policyname = 'Admins can create edit history') THEN
    CREATE POLICY "Admins can create edit history" ON public.fund_edit_history FOR INSERT 
