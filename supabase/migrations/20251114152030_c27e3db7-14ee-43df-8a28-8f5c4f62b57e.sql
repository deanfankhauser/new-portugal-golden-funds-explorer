-- Add last_email_sent_at to get_fund_manager_sign_ins()
...
BEGIN
  ...
  SELECT 
    ...
    MAX(fmel.sent_at) AS last_email_sent_at,
  ...
  LEFT JOIN public.fund_page_views fpv
    ON fpv.fund_id = f.id
  LEFT JOIN public.fund_manager_email_logs fmel
    ON fmel.fund_id = f.id
