  ...
  const fetchFunds = async () => {
    try {
      ...
      if (fetchError) {
        ...
        // If we hit auth issues (commonly 401) against the configured project,
        // try proxying via the get-develop-funds edge function which reads from Funds_Develop.
        try {
          const { data: proxyResp, error: proxyErr } = await supabase.functions.invoke('get-develop-funds');
          if (!proxyErr && proxyResp && Array.isArray(proxyResp.data)) {
            const devFunds = proxyResp.data as any[];
            const transformedFunds: Fund[] = devFunds.map((fund: any) => ({
              id: fund.id,
              name: fund.name,
              ...
              geographicAllocation: Array.isArray(fund.geographic_allocation) ? (fund.geographic_allocation as GeographicAllocation[]) : undefined,
              team: Array.isArray(fund.team_members) ? (fund.team_members as TeamMember[]) : undefined,
              documents: Array.isArray(fund.pdf_documents) ? (fund.pdf_documents as PdfDocument[]) : undefined,
              faqs: Array.isArray(fund.faqs) ? (fund.faqs as FAQItem[]) : undefined,
            ...
            // Try to fetch and apply edit history overlay as well (best effort)
            try {
            ...
            setError(null);
            console.log('✅ Loaded funds via get-develop-funds proxy:', transformedFunds.length)
            return;
          } else {
            console.warn('Proxy to get-develop-funds failed:', proxyErr || proxyResp)
          }
        } catch (proxyException) {
          console.warn('Proxy exception:', proxyException)
        }
