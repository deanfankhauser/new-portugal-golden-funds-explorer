...
interface ManagerEnquirySectionProps {
...
export const ManagerEnquirySection: React.FC<ManagerEnquirySectionProps> = ({ managerProfile, managerName }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Insert enquiry directly into database
      const { error } = await supabase.from('fund_enquiries').insert({
        fund_id: 'manager_enquiry', // Special identifier for manager enquiries
        first_name: formData.firstName,
        last_name: formData.lastName,
        email: formData.email,
        phone: formData.phone || null,
        investment_amount_range: formData.investmentAmountRange,
        interest_areas: formData.interestAreas,
        message: `[Manager Enquiry for ${managerName}]\n\n${formData.message}`,
  ...
  return (
    <Card id="contact-form" className="bg-card border border-border/40 rounded-2xl shadow-sm scroll-mt-28 md:scroll-mt-24 relative">
      {/* Loading Overlay */}
      {isSubmitting && (
          ...
          <div className="flex flex-col items-center gap-6 text-center px-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-lg text-muted-foreground animate-fade-in">
                Your enquiry has been sent to {managerName}
              </p>
      ...
      <CardHeader className="p-10 pb-8">
        <CardTitle className="text-[32px] font-semibold tracking-tight leading-tight mb-3">Get in Touch with {managerName}</CardTitle>
        <CardDescription className="text-base text-muted-foreground leading-relaxed">
          Interested in learning more about investment opportunities with <strong>{managerName}</strong>? Send us an enquiry and we'll respond within 24-48 hours.
        </CardDescription>
      </CardHeader>
