...
const CompanyLeadsTab: React.FC<CompanyLeadsTabProps> = ({ profileId, companyName }) => {
  ...
  useEffect(() => {
    const setupSubscription = async () => {
      ...
      // Set up real-time subscription for all company enquiries
      const channel = supabase
        .channel('company_enquiries_changes')
        .on(
          'postgres_changes',
          {
            event: '*',
            ...
            filter: `manager_name=eq.${companyName}`,
          },
          () => {
            fetchEnquiries();
          }
  ...
  const fetchEnquiries = async () => {
    try {
      ...
      const { data: companyFunds, error: fundsError } = await supabase.rpc('get_funds_by_company_name', {
      ...
      if (fundIds.length > 0) {
        // Include both fund-specific leads AND general company leads
        query = query.or(`fund_id.in.(${fundIds.join(',')}),and(fund_id.is.null,manager_name.eq.${companyName})`);
      } else {
        // Only general company leads if no funds exist
        query = query.eq('manager_name', companyName).is('fund_id', null);
      }
  ...
  const exportToCSV = () => {
    const headers = ['Name', 'Email', 'Phone', 'Source', 'Investment Range', 'Status', 'Date', 'Message'];
    ...
    a.href = url;
    a.download = `company-leads-${companyName.replace(/\s+/g, '-')}-${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  };
  ...
  const filteredEnquiries = enquiries
    .filter(e => {
      ...
      if (filterSource !== 'all') {
        if (filterSource === 'fund' && !e.fund_id) return false;
        if (filterSource === 'general' && e.fund_id) return false;
      }
      if (searchQuery) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">All funds & general</p>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <CardTitle>Company Lead Management</CardTitle>
              <CardDescription>All enquiries for {companyName}</CardDescription>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <Select value={filterSource} onValueChange={setFilterSource}>
              ...
              <SelectContent>
                ...
                <SelectItem value="fund">Fund Enquiries</SelectItem>
          ...
          {filteredEnquiries.length === 0 ? (
            <div className="text-center py-12 border border-border/40 rounded-lg shadow-sm bg-card">
              ...
              <p className="text-muted-foreground max-w-md mx-auto">
                Leads will appear here once prospective investors enquire about your funds or company.
              </p>
            </div>
          ) : (
                        ...
                        <TableCell>
                          {enquiry.fund_id ? (
                            <Badge variant="secondary" className="text-xs">
                              {enquiry.funds?.name || 'Fund Enquiry'}
                            </Badge>
                          ) : (
                        ...
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          ...
                          >
                            <div className="flex items-center space-x-1">
                              <RadioGroupItem value="open" id={`open-${enquiry.id}`} className="h-4 w-4" />
                              <label htmlFor={`open-${enquiry.id}`} className="text-xs cursor-pointer">Open</label>
                            </div>
                            <div className="flex items-center space-x-1">
                              <RadioGroupItem value="won" id={`won-${enquiry.id}`} className="h-4 w-4" />
                              <label htmlFor={`won-${enquiry.id}`} className="text-xs cursor-pointer">Won</label>
                            </div>
                            <div className="flex items-center space-x-1">
                              <RadioGroupItem value="closed_lost" id={`lost-${enquiry.id}`} className="h-4 w-4" />
                              <label htmlFor={`lost-${enquiry.id}`} className="text-xs cursor-pointer">Closed Lost</label>
                            </div>
                          </RadioGroup>
                        </TableCell>
                        <TableCell>
...
export default CompanyLeadsTab;
