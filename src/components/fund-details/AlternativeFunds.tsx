...
const AlternativeFunds: React.FC<AlternativeFundsProps> = ({ currentFund }) => {
  ...
  return (
    <section className="bg-background rounded-2xl border border-border overflow-hidden shadow-sm">
      {/* Header */}
      <div className="px-8 sm:px-10 py-10 bg-gradient-to-b from-muted/30 to-background border-b border-border">
      ...
      <div className="p-6 space-y-4">
        {alternativeFunds.map((fund) => (
          <article
            key={fund.id}
            className="group relative bg-background border border-border rounded-xl p-7 transition-all duration-200 hover:border-primary/30 hover:shadow-lg hover:translate-y-[-2px]"
          >
      ...
      {/* Footer Actions */}
      <div className="px-8 sm:px-10 py-8 bg-muted/20 border-t border-border flex flex-col sm:flex-row justify-between items-center gap-6">
        <Link
