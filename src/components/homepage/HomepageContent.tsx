
import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { Fund } from '../../data/funds';
import FundListItem from '../FundListItem';
import FundFilter from '../FundFilter';
import PremiumCTA from '../cta/PremiumCTA';
import FundListSkeleton from '../common/FundListSkeleton';
import { FundTag } from '../../data/funds';
import { Button } from '@/components/ui/button';
import { ClipboardCheck, Filter, X } from 'lucide-react';

interface HomepageContentProps {
  filteredFunds: Fund[];
  selectedTags: FundTag[];
  setSelectedTags: (tags: FundTag[]) => void;
  searchQuery: string;
  setSearchQuery: (query: string) => void;
  isAuthenticated: boolean;
}

const HomepageContent: React.FC<HomepageContentProps> = ({
  filteredFunds,
  selectedTags,
  setSelectedTags,
  searchQuery,
  setSearchQuery,
  isAuthenticated
}) => {
  const [showMobileFilter, setShowMobileFilter] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  return (
    <div className="space-y-4 sm:space-y-6 lg:space-y-8">
      {/* Mobile Filter Toggle Button - Enhanced mobile design */}
      <div className="lg:hidden">
        <Button
          onClick={() => setShowMobileFilter(!showMobileFilter)}
          variant="outline"
          className="w-full justify-center gap-2 sm:gap-3 bg-white border-gray-200 hover:bg-gray-50 h-12 sm:h-14 text-sm sm:text-base font-medium"
        >
          {showMobileFilter ? (
            <>
              <X className="h-4 w-4 sm:h-5 sm:w-5" />
              Hide Filters
            </>
          ) : (
            <>
              <Filter className="h-4 w-4 sm:h-5 sm:w-5" />
              Show Filters & Search
            </>
          )}
        </Button>
      </div>

      {/* Mobile Filter Section - Enhanced spacing */}
      {showMobileFilter && (
        <div className="lg:hidden">
          <div className="bg-white p-4 sm:p-6 rounded-xl border border-gray-200 shadow-sm">
            <FundFilter
              selectedTags={selectedTags}
              setSelectedTags={setSelectedTags}
              searchQuery={searchQuery}
              setSearchQuery={setSearchQuery}
            />
          </div>
        </div>
      )}

      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
        {/* Left Sidebar - Enhanced mobile responsiveness */}
        <div className="lg:col-span-1 order-2 lg:order-1 hidden lg:block">
          <div className="lg:sticky lg:top-4 space-y-4 sm:space-y-6">
            {/* Fund Quiz CTA - Enhanced typography and spacing */}
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-5 sm:p-6 rounded-xl border border-blue-200 shadow-sm">
              <div className="flex items-start gap-3 sm:gap-4 mb-4">
                <div className="bg-blue-100 p-2.5 sm:p-3 rounded-lg flex-shrink-0">
                  <ClipboardCheck className="h-5 w-5 sm:h-6 sm:w-6 text-blue-600" />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-blue-900 mb-2 text-base sm:text-lg lg:text-xl leading-tight">
                    Not sure which fund is right for you?
                  </h3>
                  <p className="text-sm sm:text-base text-blue-700 mb-4 sm:mb-5 leading-relaxed">
                    Take our quick quiz to get personalized recommendations based on your investment goals
                  </p>
                </div>
              </div>
              <Link to="/fund-quiz" className="block">
                <Button className="w-full bg-blue-600 hover:bg-blue-700 text-sm sm:text-base h-12 sm:h-14 font-medium transition-all duration-200">
                  <ClipboardCheck className="mr-2 h-4 w-4 sm:h-5 sm:w-5" />
                  Take Fund Quiz
                </Button>
              </Link>
            </div>
            
            {/* Filter Section - Enhanced spacing */}
            <div className="bg-white p-5 sm:p-6 rounded-xl border border-gray-200 shadow-sm">
              <FundFilter
                selectedTags={selectedTags}
                setSelectedTags={setSelectedTags}
                searchQuery={searchQuery}
                setSearchQuery={setSearchQuery}
              />
            </div>
          </div>
        </div>
        
        {/* Main Content - Enhanced loading states and responsiveness */}
        <div className="lg:col-span-3 order-1 lg:order-2">
          {isLoading ? (
            <FundListSkeleton />
          ) : filteredFunds.length === 0 ? (
            <div className="text-center py-12 sm:py-16 lg:py-20 bg-white rounded-xl shadow-sm border p-6 sm:p-8 lg:p-12">
              <div className="max-w-md mx-auto">
                <div className="bg-gray-100 w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
                  <ClipboardCheck className="h-8 w-8 sm:h-10 sm:w-10 text-gray-400" />
                </div>
                <h3 className="text-lg sm:text-xl lg:text-2xl font-semibold mb-3 sm:mb-4 text-gray-900">No funds found</h3>
                <p className="text-gray-500 mb-6 sm:mb-8 text-sm sm:text-base lg:text-lg leading-relaxed">
                  Try adjusting your filters or search query to find funds that match your criteria
                </p>
                <Link to="/fund-quiz">
                  <Button variant="outline" className="text-sm sm:text-base h-12 sm:h-14 px-6 sm:px-8 font-medium">
                    <ClipboardCheck className="mr-2 h-4 w-4 sm:h-5 sm:w-5" />
                    Take Quiz for Recommendations
                  </Button>
                </Link>
              </div>
            </div>
          ) : (
            <>
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 lg:mb-8 gap-2 sm:gap-4">
                <p className="text-gray-600 font-medium text-sm sm:text-base lg:text-lg">
                  {filteredFunds.length} fund{filteredFunds.length !== 1 ? 's' : ''} found
                </p>
                <div className="text-xs sm:text-sm lg:text-base text-gray-500">
                  Showing all available funds
                </div>
              </div>
              <div className="space-y-4 sm:space-y-6 lg:space-y-8">
                {filteredFunds.map((fund, index) => (
                  <div key={fund.id}>
                    <FundListItem fund={fund} />
                    {/* Insert Premium CTA with enhanced spacing */}
                    {!isAuthenticated && (index + 1) % 3 === 0 && index < filteredFunds.length - 1 && (
                      <div className="my-6 sm:my-8 lg:my-10">
                        <PremiumCTA variant="full" location={`homepage-after-fund-${index + 1}`} />
                      </div>
                    )}
                  </div>
                ))}
              </div>
            </>
          )}
        </div>
      </div>

      {/* Mobile Fund Quiz CTA - Enhanced mobile design */}
      <div className="lg:hidden">
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-5 sm:p-6 rounded-xl border border-blue-200 shadow-sm">
          <div className="flex items-start gap-3 sm:gap-4 mb-4">
            <div className="bg-blue-100 p-2.5 sm:p-3 rounded-lg flex-shrink-0">
              <ClipboardCheck className="h-5 w-5 sm:h-6 sm:w-6 text-blue-600" />
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-blue-900 mb-2 text-base sm:text-lg leading-tight">
                Not sure which fund is right for you?
              </h3>
              <p className="text-sm sm:text-base text-blue-700 mb-4 sm:mb-5 leading-relaxed">
                Take our quick quiz to get personalized recommendations based on your investment goals
              </p>
            </div>
          </div>
          <Link to="/fund-quiz" className="block">
            <Button className="w-full bg-blue-600 hover:bg-blue-700 text-sm sm:text-base h-12 sm:h-14 font-medium transition-all duration-200">
              <ClipboardCheck className="mr-2 h-4 w-4 sm:h-5 sm:w-5" />
              Take Fund Quiz
            </Button>
          </Link>
        </div>
      </div>
    </div>
  );
};

export default HomepageContent;
